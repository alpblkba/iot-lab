...
static uint32_t compute_centered_rms(const int16_t *samples, uint32_t count)
{
    int64_t sum = 0;
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        square_sum += (uint64_t)((int64_t)centered * (int64_t)centered);
    }

    return isqrt_u64(square_sum / count);
}

static void audio_report_rms(const int16_t *samples, uint32_t count, const char *tag)
{
    char line[80];
...
static void process_microphone_audio(void)
{
    ...
    if (audio_half_ready != 0U) {
        audio_half_ready = 0U;
        audio_report_rms(&audio_buffer[0], AUDIO_BUFFER_SAMPLES / 2U, "half");
    }

    if (audio_full_ready != 0U) {
        audio_full_ready = 0U;
        audio_report_rms(&audio_buffer[AUDIO_BUFFER_SAMPLES / 2U], AUDIO_BUFFER_SAMPLES / 2U, "full");
    }
}
