...
static char internal_data[] = "CODEADBEEF";

typedef struct {
    char text[11]; // Enough space for internal_data and temrinating null byte
    uint32_t checksum;
} Record;
...
/* Because we are safety conscious we want to verify that we copied the correct string
*/
static uint32_t checksum_string(const char *s)
{
    uint32_t sum = 0;
    size_t len = strlen(s);
...
static void dma_copy(void)
{
    ...
    /*
     * This is intended to behave like:
     *
     *   memcpy(rec.text, internal_data, strlen(internal_data));
     *
     * If the copied string is corrupted, check whether the DMA channel
     * configuration really matches a byte-wise char-buffer copy.
     */
    if (HAL_DMA_Start_IT(&handle_GPDMA1_Channel0,
                         (uint32_t)internal_data,
                         (uint32_t)rec.text,
                         strlen(internal_data)) != HAL_OK) {
        Error_Handler();
    }
}
...
{
    /*
     * The checksum is computed after the DMA transfer completed.
     * If this fails, inspect rec.text in memory before changing this function.
     */
    rec.checksum = checksum_string(rec.text);

    if (rec.checksum != checksum_string(internal_data)) {
        Error_Handler();
    }

    sprintf(formatted, "Record: [%s]", rec.text);
}
...
int main(void)
{
  ...
  // memset(&rec, 0x41, sizeof(rec));
  // memset(guard, 'A', sizeof(guard));
...
static void MX_GPDMA1_Init(void)
{
  ...
  HAL_DMA_RegisterCallback(&handle_GPDMA1_Channel0, HAL_DMA_XFER_CPLT_CB_ID, XferCpltCallback);
