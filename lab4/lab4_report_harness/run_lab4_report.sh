...
build_project() {
  ...
  log "build $name ($CONFIG)"
  (
    cd "$project_dir"
    rm -rf build

    if cmake --list-presets 2>/dev/null | grep -q '"'"$CONFIG"'"'; then
      cmake --preset "$CONFIG" "${cmake_args[@]}"
      cmake --build --preset "$CONFIG"
    else
      echo "No CMake preset named $CONFIG; using Debug preset with CMAKE_BUILD_TYPE=$CONFIG"
      cmake --preset Debug -DCMAKE_BUILD_TYPE="$CONFIG" "${cmake_args[@]}"
      cmake --build --preset Debug
    fi
  ) 2>&1 | tee "$RUN_DIR/${name}_build.log"

  local elf
  elf="$(find "$project_dir/build" -name '*.elf' -type f | sort | head -n 1)"
  if [[ -z "$elf" ]]; then
    echo "No ELF produced for $name" >&2
    exit 1
  fi
  echo "$elf" > "$RUN_DIR/${name}_elf.txt"
  printf '%s\n' "$elf"
}
